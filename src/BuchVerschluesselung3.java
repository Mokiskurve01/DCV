public class BuchVerschluesselung3 {
    /***
     * Es soll ein Programm entworfen werden,
     * welches mit Hilfe des Hermann Hesse Textes aus einer der vorhergehenden Übungen
     * das Wort "Java" mit Hilfe der Buch-Verschlüsselung
     * (https://de.wikipedia.org/wiki/Buch-Verschlüsselung) verschlüsselt.
     */
    private final static String buch =
    ...
    public static void main(String[] args) {
        System.out.println(verschluesseln("Java"));
        System.out.println(entschluesseln("381 34 531 31"));
    }

    public static String verschluesseln(String text) {
        String verschluesselttext = "";
        int pos = 0;
        int lastPos = 0;
        for (int i = 0; i < text.length(); i++) {
            pos = buch.indexOf(text.charAt(i), pos);
            verschluesselttext += pos - lastPos + " ";
            lastPos = pos;
        }
        return verschluesselttext;
    }

    public static String entschluesseln(String verschluesselttext) {
        String entschluesselnText = "";
        int lastPos = 0;
        String[] worte = verschluesselttext.split(" ");
        for (int i = 0; i < worte.length; i++) {
            int position = Integer.valueOf(worte[i]);
            entschluesselnText += buch.charAt(position + lastPos);
            lastPos = position + lastPos;
        }
        return entschluesselnText;
    }
}
